...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select distinct movies.id, movies.title, movies.year, movies.genres from movies join ratings on ratings.movie_id = movies.id order by movies.year, movies.title limit 10;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select users.name, movies.title, movies.year, ratings.rating, DATE(ratings.timestamp, 'unixepoch') as date from ratings join users on ratings.user_id = users.id join movies on ratings.movie_id = movies.id order by users.name, movies.title, ratings.rating limit 50;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select movies.id, movies.title, movies.year, movies.genres, tags.tag from movies join tags on tags.movie_id = movies.id order by movies.year, movies.title, tags.tag limit 40;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select movies.title, movies.year, movies.genres, users.gender, ratings.rating, count(ratings.id) as count from movies join ratings on ratings.movie_id = movies.id join users on ratings.user_id = users.id where movies.genres like '%%Comedy%%'and movies.year > 2000 and ratings.rating >= 4.5 and users.gender = 'male' group by movies.title order by movies.year, movies.title;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select occupation, count(distinct id) as sum from users group by occupation;" 
sqlite3 movies_rating.db -box -echo "select occupation, max(number) as max from (select occupation, count(distinct id) as number from users group by occupation);"
sqlite3 movies_rating.db -box -echo "select occupation, min(number) as min from (select occupation, count(distinct id) as number from users group by occupation);"
